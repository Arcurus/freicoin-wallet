...
public final class BitcoinIntegration
{
	...
	private static final String MIMETYPE_PAYMENTREQUEST = "application/freicoin-paymentrequest"; // BIP 71
	...
	{
		final StringBuilder uri = new StringBuilder("freicoin:");
		if (address != null)
			uri.append(address);
		if (amount != null)
	...
	{
		Toast.makeText(context, "No Freicoin application found.\nPlease install Freicoin Wallet.", Toast.LENGTH_LONG).show();

		final Intent marketIntent = new Intent(Intent.ACTION_VIEW, Uri.parse("market://details?id=cc.freicoin.wallet"));
		final Intent binaryIntent = new Intent(Intent.ACTION_VIEW, Uri.parse("https://github.com/Skirmant/freicoin-wallet/releases"));
